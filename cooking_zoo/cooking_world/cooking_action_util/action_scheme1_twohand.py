...
"""
Mostly copied from action_scheme1

Only differences should be in resolve_interaction(), which considers the explicit multihanded actions
"""
...
def resolve_interaction(world, agent, action):
    ...
    elif action == world.action_scheme.INTERACT_ARM1:
        world.resolve_primary_interaction(agent, 0)
    elif action == world.action_scheme.INTERACT_PICK_UP_SPECIAL_ARM1:
        world.resolve_interaction_pick_up_special(agent, 0)
    elif action == world.action_scheme.EXECUTE_ACTION_ARM1:
        world.resolve_execute_action(agent, 0)

    elif action == world.action_scheme.INTERACT_ARM2:
        world.resolve_primary_interaction(agent, 1)
    elif action == world.action_scheme.INTERACT_PICK_UP_SPECIAL_ARM2:
        world.resolve_interaction_pick_up_special(agent, 1)
    elif action == world.action_scheme.EXECUTE_ACTION_ARM2:
        world.resolve_execute_action(agent, 1)
